...
describe('decode block', () => {
  test('decodes stacks 2.x block with burnchain op tx', () => {
    ...
    expect(block.transactions.map(tx => tx.tx.tx_index)).toStrictEqual([1, 2]);
    expect(block.transactions.map(tx => tx.tx.txid)).toStrictEqual([
    ...
    expect(block.transactions[0].events).toStrictEqual([]);
    expect(block.transactions[1].events.map(event => event.event_index)).toStrictEqual([1]);
  });
});
